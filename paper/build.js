...
const CITE=(n)=>new TextRun({text:n,font:SER,size:20,superScript:true});
const PC=(parts,o={})=>new Paragraph({
  spacing:{after:o.after??120,line:o.line??276},alignment:o.align,
  children:parts.map(p=>typeof p==="string"
    ? new TextRun({text:p,font:SER,size:20})
    : new TextRun({text:p.c,font:SER,size:20,superScript:true}))});
const REF=(n,t)=>new Paragraph({
  spacing:{after:60,line:240},indent:{left:340,hanging:340},
  children:[new TextRun({text:n+". ",font:SER,size:18}),
            new TextRun({text:t,font:SER,size:18})]});
...
P("Reduced-order models accelerate simulation by evolving a small subspace of a "+
  ...
  "without exception over 300 test windows, and an exact identity that predicts the "+
  "achievable improvement from two computable quantities. The identity holds to "+
  "at most 5.4 × 10⁻¹² across four systems spanning quadratic and cubic "+
  ...
  "observations the method never sees. The approach corrects reduction error "+
  "and assumes the governing equations are correct: under deliberate "+
  "model-form error its predictions become optimistic, which bounds its scope "+
  "and identifies observation-driven assimilation as its complement rather "+
  "than its competitor.",{align:AlignmentType.JUSTIFIED}),
...
P("Simulating a turbulent flow, a combustor, or a climate subsystem at full "+
  ...
  "low-dimensional surrogate for the full state\u00b9\u00b7\u00b2. Every such surrogate is "+
  "wrong, and a large literature addresses how wrong: a posteriori error "+
  "estimators certify accuracy\u00b3\u2074, adjoint-weighted residuals identify where "+
  "refinement pays\u2075\u2076, and closure models supply the terms projection "+
  "discards\u2077\u2078. What unites these approaches is that they estimate or bound an "+
  "error magnitude in order to improve the model.",{align:AlignmentType.JUSTIFIED}),
...
P("The bound binds only when the reduced subspace is impoverished by "+
  "construction. Probing the dynamical transport operator T = P·DΦ·Q directly "+
  "and projecting the error forecast onto its range gives a reachable fraction "+
  "of exactly 1.000 in every window tested (n = 63, four dynamical regimes), "+
  "with the rank of T equal to the full observable dimension. A map from "+
  "roughly four hundred unresolved coordinates into twelve reported ones is "+
  "generically surjective, so every reported direction is reachable by some "+
  "hidden correction and the ceiling is vacuous. Correctability is therefore "+
  "not limited by WHICH directions a hidden correction can reach; it is "+
  "limited by the size and alignment of the response, which is what the next "+
  "section quantifies.",{align:AlignmentType.JUSTIFIED}),
...
P("The observation operator generalises to instrument readings. Replacing the "+
  ...
  "in 36 of 36 windows. We note that the baseline model error in this "+
  "configuration is 0.56%, an order of magnitude below the 1.8–11.3% band of "+
  "the frozen-basis test above; the result demonstrates the observation "+
  "operator at three-dimensional scale on an already accurate model, and a "+
  "repetition at larger baseline error would strengthen it.",{align:AlignmentType.JUSTIFIED}),
...
H("The scope limit: correct equations",HeadingLevel.HEADING_2),
P("Every experiment above generates the reference trajectory from the same "+
  "governing equations the reduced model projects, so the only error present "+
  "is reduction error. Deployed models are not generally like that: they carry "+
  "mis-specified coefficients and missing terms. The distinction matters "+
  "because the defect of equation (1) then contains two different objects. "+
  "Reduction error is a state error and is correctable. Model-form bias is a "+
  "persistent difference between two equations, and no change of state can "+
  "remove it.",{align:AlignmentType.JUSTIFIED}),
P("We tested this by evolving the reference under the exact equation while the "+
  "model — both the reduced model and the full map inside the estimator — used "+
  "a single coefficient scaled by 1 + δ, for each of the three terms in turn "+
  "(n = 100 per term). Two things survive and one does not. The causal "+
  "signature is unaffected: the correctly signed correction beat its reverse "+
  "in 20 of 20 windows at every δ in every term, so hidden state error remains "+
  "real and remains transported even when the equations are wrong. The gain "+
  "degrades, as it must, because a growing share of the reported error is bias "+
  "rather than state error.",{align:AlignmentType.JUSTIFIED}),
P("The predictor, however, does not stay honest. At δ = 0.10 the method "+
  "retains roughly a tenth of its benefit while reporting that it retains "+
  "about two thirds; the ratio of predicted to realised gain rises from 0.99 "+
  "at δ = 0 to 6.1 and 6.3 for the two dissipative terms. The reason is "+
  "structural rather than a defect of implementation: the bias enters the "+
  "reduced model and the estimator identically, so the method is internally "+
  "consistent and cannot detect the inconsistency. It corrects accurately "+
  "toward its own attractor, which is no longer the true one. Perturbing the "+
  "transport coefficient is the mild case, at 2.3, so model error in the "+
  "dissipative terms is what breaks calibration.",{align:AlignmentType.JUSTIFIED}),
P("This bounds the method precisely. It addresses reduction error and assumes "+
  "the governing equations are correct, and its trust gate is calibrated "+
  "against reduction error alone. Detecting model-form error requires "+
  "information the method deliberately does not use — which is exactly what "+
  "observations supply. That makes observation-driven assimilation the natural "+
  "complement to this approach rather than its competitor.",
  {align:AlignmentType.JUSTIFIED}),
...
P("The most consequential open question is what sets λ, and five attempts to "+
  "answer it place the obstruction in one place. Response alignment against "+
  "observable dimension, and an advective-locality group built from transport "+
  "distance and sensor spacing, were both tested and rejected. A closed form "+
  "in the hidden-driven share failed, and failed further when its neglected "+
  "term was restored. The geometric route is vacuous, as shown above: the "+
  "reachable set is the whole reported space.",{align:AlignmentType.JUSTIFIED}),
P("What does survive is an empirical transfer. The ratio of hidden-block to "+
  "observable-block response is computable without a reference, tracks its own "+
  "truth-free estimate at Spearman 1.0000, and orders λ across a 24,000-fold "+
  "range. Fitting a single monotone calibration of λ against that ratio on "+
  "Kuramoto–Sivashinsky alone, freezing it, and applying it without refitting "+
  "to complex Ginzburg–Landau and to the finite-difference sensor system "+
  "(n = 89 held out) orders λ at Spearman 0.72 and halves the error of a "+
  "constant baseline. But the calibration over-predicts the pointwise-"+
  "observable system by a factor of about 2.5 while transferring accurately "+
  "across the quadratic-to-cubic boundary.",{align:AlignmentType.JUSTIFIED}),
P("The failure is therefore the same boundary reported above, reached by an "+
  "independent route: λ transfers across equations and nonlinearity classes "+
  "and not across observation-operator classes. A predictive theory of "+
  "correctability, as distinct from a transferable ordering, remains open, and "+
  "the evidence indicates it will have to account for the observation operator "+
  "rather than the dynamics alone.",{align:AlignmentType.JUSTIFIED}),
...
P("Throughout, Δ denotes the FINITE-AMPLITUDE observable response to the "+
  ...
  {align:AlignmentType.JUSTIFIED}),
H("Model-form error",HeadingLevel.HEADING_2),
P("The reference was evolved under the exact Kuramoto–Sivashinsky equation "+
  "while the reduced model and the full map inside the estimator both used a "+
  "single coefficient scaled by 1 + δ, applied in turn to the advective, "+
  "diffusive and hyperdiffusive terms, with δ ∈ {0, 0.005, 0.02, 0.05, 0.10} "+
  "and n = 100 windows per term. δ = 0 recovers the configuration used "+
  "elsewhere. The reported calibration ratio is the median of predicted over "+
  "realised gain across windows.",{align:AlignmentType.JUSTIFIED}),
H("Transport-operator reachability",HeadingLevel.HEADING_2),
P("The finite-time operator T = P·DΦ·Q was probed with 24 random orthonormal "+
  "directions in the unresolved block at relative amplitude 10⁻⁶, the "+
  "responses assembled in the packed observable coordinates, and an orthonormal "+
  "basis for the range obtained by singular value decomposition. The reachable "+
  "fraction is the norm of the projection of the truth-free error forecast onto "+
  "that range, divided by its own norm. Singular values below 10⁻¹⁰ of the "+
  "leading one were discarded.",{align:AlignmentType.JUSTIFIED}),
H("Blind calibration transfer",HeadingLevel.HEADING_2),
P("A two-parameter monotone map λ = (aκ)/((aκ)^p + 1)^(1/p), chosen for its "+
  "limits rather than its flexibility, was fitted by median absolute deviation "+
  "on Kuramoto–Sivashinsky windows alone, written to disk, and applied without "+
  "refitting to the held-out systems. Predictions were recorded before the "+
  "corresponding corrections were scored. The constant baseline against which "+
  "it is compared predicts the median calibration-set λ for every held-out "+
  "window.",{align:AlignmentType.JUSTIFIED}),
...
P("The ensemble variational baseline is deliberately advantaged, and its exact "+
  "information budget should be stated. Its background covariance is estimated "+
  "from an ensemble of past error snapshots U(t_j) - V(t_j), which are formed "+
  "using the reference trajectory; it additionally consumes the true observable "+
  "at the analysis time. It therefore uses strictly more external information "+
  "than the method it is compared against, which uses none. It applies the "+
  "analysis increment "+
  ...
  "10⁻⁸, 10⁻⁴ and 10⁻², yielding gains of 1.26, 1.20 and 1.12. Because the "+
  "baseline is advantaged in this way, the comparison establishes that "+
  "hidden-state correction is competitive under a strictly smaller information "+
  "budget; it does not establish superiority over data assimilation in general.",
  {align:AlignmentType.JUSTIFIED}),
...
REF(1,"Benner, P., Ohlberger, M., Cohen, A. & Willcox, K. (eds) Model Reduction and Approximation: Theory and Algorithms (SIAM, Philadelphia, 2017)."),
REF(2,"Kramer, B., Peherstorfer, B. & Willcox, K. E. Learning nonlinear reduced models from data with operator inference. Annu. Rev. Fluid Mech. 56, 521–548 (2024)."),
REF(3,"Homescu, C., Petzold, L. R. & Serban, R. Error estimation for reduced-order models of dynamical systems. SIAM J. Numer. Anal. 43, 1693–1714 (2005)."),
REF(4,"Feng, L., Chellappa, S. & Benner, P. A posteriori error estimation for model order reduction of parametric systems. Adv. Model. Simul. Eng. Sci. 11, 5 (2024)."),
REF(5,"Blonigan, P. J. & Parish, E. J. Evaluation of dual-weighted residual and machine learning error estimation for projection-based reduced-order models of steady partial differential equations. Comput. Methods Appl. Mech. Eng. 409, 115988 (2023)."),
REF(6,"Carlberg, K. Adaptive h-refinement for reduced-order models. Int. J. Numer. Methods Eng. 102, 1192–1210 (2015)."),
REF(7,"Parish, E. J. & Duraisamy, K. Non-Markovian closure models for large eddy simulations using the Mori–Zwanzig formalism. Phys. Rev. Fluids 2, 014604 (2017)."),
REF(8,"Wang, Q., Ripamonti, N. & Hesthaven, J. S. Recurrent neural network closure of parametric POD-Galerkin reduced-order models based on the Mori–Zwanzig formalism. J. Comput. Phys. 410, 109402 (2020)."),
REF(9,"Chellappa, S., Feng, L. & Benner, P. Accurate error estimation for model reduction of nonlinear dynamical systems via data-enhanced error closure. Comput. Methods Appl. Mech. Eng. 420, 116712 (2024)."),
REF(10,"Law, K., Stuart, A. & Zygalakis, K. Data Assimilation: A Mathematical Introduction (Springer, Cham, 2015)."),
REF(11,"Asch, M., Bocquet, M. & Nodet, M. Data Assimilation: Methods, Algorithms, and Applications (SIAM, Philadelphia, 2016)."),
REF(12,"Kuramoto, Y. & Tsuzuki, T. Persistent propagation of concentration waves in dissipative media far from thermal equilibrium. Prog. Theor. Phys. 55, 356–369 (1976)."),
REF(13,"Sivashinsky, G. I. Nonlinear analysis of hydrodynamic instability in laminar flames. Acta Astronaut. 4, 1177–1206 (1977)."),
REF(14,"Aranson, I. S. & Kramer, L. The world of the complex Ginzburg–Landau equation. Rev. Mod. Phys. 74, 99–143 (2002)."),
REF(15,"Kalman, R. E. On the general theory of control systems. IFAC Proc. Vol. 1, 491–502 (1960)."),
REF(16,"Parish, E. J. & Duraisamy, K. A dynamic subgrid scale model for large eddy simulations based on the Mori–Zwanzig formalism. J. Comput. Phys. 349, 154–175 (2017)."),
REF(17,"Drohmann, M. & Carlberg, K. The ROMES method for statistical modeling of reduced-order-model error. SIAM/ASA J. Uncertain. Quantif. 3, 116–145 (2015)."),

...
ax2.set_ylim(0,2.30)
for x,g,n in zip(range(3),gains,["36/36","0/36","36/36"]):
...
pts=[(0.0891,1.0350,"KS unstable",C["ks"]),
     (0.4445,1.6510,"Navier–Stokes",C["ns"]),
     (0.3909,1.8811,"FD + sensors",C["fd"]),
     (1.0298,18.63,"KS damped",C["ks"])]
for l,g,n,c in pts:
...
sysn=["Kuramoto–\nSivashinsky","Navier–\nStokes","Ginzburg–\nLandau\n(cubic)",
      "FD + point\nsensors"]
errs=[1.97e-15,2.31e-14,5.38e-12,1.12e-15]
...
V=[("A",0.766,1.0544,C["red"]),("B",1.0,1.5834,C["ns"]),
   ("C",1.0,1.5978,C["ns"]),("D",1.0,1.5849,C["ns"]),("E",1.0,1.5639,C["ns"])]
for n,rho,g,c in V:
    x=min(rho,0.995)
    ax.scatter([x],[g],s=36,color=c,zorder=5,edgecolor="white",lw=0.6)
ax.annotate("A  POD only\nreachability-limited",(0.766,1.054),(0.30,3.2),
...
ax.text(0.98,1.02,"300 windows, no violations",fontsize=6.3,
...
ns=np.array([0,1,2,3,6,12,18,24]); fr=ns/24
er=np.array([0.0049,0.2231,0.3066,0.3777,0.5133,0.5352,0.4160,0.4509])
lm=np.array([0.9201,0.6537,0.6126,0.4783,0.3747,0.3329,0.4057,0.3560])
...
ax1.annotate("one sensor in 24\ngives 49% of the\nsaturated error",
             (1/24,0.2231),(0.20,0.13),fontsize=6.3,color=C["fd"],
...
meth=["uncorrected","hidden-\nonly","observable-\nonly",
      "ensemble\n3DVar*","adaptive\n(ours)"]
unst=[1.0,1.05,3.11,3.03,14.09]; damp=[1.0,16.34,1.00,1.28,16.65]
x=np.arange(5); w=0.30
ax1.bar(x-w/2,unst,w,label="unstable band  ($\\lambda\\approx0.09$)",
        color=C["ks"],zorder=3)
ax1.bar(x+w/2,damp,w,label="damped band  ($\\lambda\\approx1.04$)",
        color=C["ns"],zorder=3)
...
for xi,(u,d) in enumerate(zip(unst,damp)):
    # 16.34 (damped, hidden-only) sits right and lower; 14.09 (unstable,
    # adaptive) sits left and lower, so neither collides with its neighbour
...
Nv=[40,64,80]; meas=[3.0613,0.7110,0.3443]; nom=[1.029,0.2512,0.1286]

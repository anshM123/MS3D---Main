#!/usr/bin/env python3
"""
M3D TEST AD3D — ADAPTIVE CORRECTION on 3D NAVIER-STOKES

Runs the 12 Test-X cases across two independent CUDA subprocesses.

WHY THIS EXISTS

Adaptive correction-subspace and amplitude selection was built and validated on
Kuramoto-Sivashinsky, where it beat every fixed strategy in both regimes and
beat ensemble 3DVar without using truth:

    band      best fixed   3DVar(truth)   ADAPTIVE
    unstable        3.25           3.03      14.09
    damped         16.34           1.28      16.65

Everything in that result is 1D. This runs the same algorithm on 3D
Navier-Stokes, which is the only testbed here at a scale anyone would deploy.

AT DECISION TIME, no truth is touched. From the defect recurrence we have
e_hat, and we split it:

    candidates:  0                do nothing
                 alpha Q e_hat    hidden-only, observable untouched (dP = 0)
                 alpha P e_hat    observable-only
                 alpha e_hat      full state
    b_hat     :  the recurrence's own forecast of future observable error
    selection :  argmin over candidates of || b_hat + Delta_hat(c) ||

Truth is used ONLY afterwards, to score the decision.

Gates:
  T1  the selector beats every FIXED strategy on median gain
  T2  it beats fixed alpha=1 Q-only, which is the published X-v3 result (1.651)
  T3  when it picks Q, the observable is preserved exactly (dP = 0.000e+00)
  T4  DESCRIPTIVE: what it picks per case, and how that tracks lambda
"""
...
def worker_main(physical_gpu: int, precision="fp64", jvp="analytic",
    ...
    POINT_FILE = SAVE_DIR_X / f"m3d_AD3D_{tag}_gpu{physical_gpu}_interventions_partial.csv"
    CASE_FILE = SAVE_DIR_X / f"m3d_AD3D_{tag}_gpu{physical_gpu}_cases_partial.csv"
    SWEEP_FILE = SAVE_DIR_X / f"m3d_AD3D_{tag}_gpu{physical_gpu}_sweep_partial.csv"
...
def finalize_outputs(tag="fp64_analytic", noise_sweep=True,
    ...
    pfiles = [SAVE_DIR / f"m3d_AD3D_{tag}_gpu{g}_interventions_partial.csv" for g in (0, 1)]
    cfiles = [SAVE_DIR / f"m3d_AD3D_{tag}_gpu{g}_cases_partial.csv" for g in (0, 1)]
    sfiles = [SAVE_DIR / f"m3d_AD3D_{tag}_gpu{g}_sweep_partial.csv" for g in (0, 1)]
    ...
    RESULTS_X.to_csv(SAVE_DIR / f"m3d_AD3D_{tag}_causal_interventions.csv", index=False)
    CASES_X.to_csv(SAVE_DIR / f"m3d_AD3D_{tag}_case_summary.csv", index=False)
    ...
    if noise_sweep and all(f.exists() for f in sfiles):
        ...
        SW.to_csv(SAVE_DIR / f"m3d_AD3D_{tag}_estimator_sweep.csv", index=False)
    ...
    with open(SAVE_DIR / f"m3d_AD3D_{tag}_COMPLETE.txt", "w") as f:
        f.write(
    ...
    print("\nOutputs:")
    print(SAVE_DIR / f"m3d_AD3D_{tag}_causal_interventions.csv")
    print(SAVE_DIR / f"m3d_AD3D_{tag}_case_summary.csv")
    if noise_sweep:
        print(SAVE_DIR / f"m3d_AD3D_{tag}_estimator_sweep.csv")
    print(SAVE_DIR / f"m3d_AD3D_{tag}_COMPLETE.txt")

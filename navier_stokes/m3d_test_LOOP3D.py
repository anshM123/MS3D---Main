#!/usr/bin/env python3
"""
M3D TEST LOOP3D — AUTONOMOUS CLOSED LOOP, wall-clock, N configurable

Runs the 12 Test-X cases across two independent CUDA subprocesses.

WHY THIS EXISTS

Every cost figure so far measures the ESTIMATOR. This measures the WHOLE
algorithm running autonomously against the solver it is meant to replace:

    V_k -> COARSE defect -> e_hat -> candidate probe -> subspace + alpha
        -> trust gate -> correction -> V_{k+1}        every segment, no truth

A CPU attempt at N=24 failed on scale and the three coarse grids showed why:
M=16 the gate declined everything (no harm, no help), M=20 fired twice and made
accuracy WORSE, M=24 was the fine grid and reconstructed truth — the Z2
artifact. At N=24 no coarse grid is simultaneously accurate enough to help and
coarse enough to matter.

The project's own wall-clock benchmark says why: the estimator costs 3.06x the
solver at N=40, 0.71x at N=64, 0.34x at N=80. The crossover is around N=64.
So this runs at BOTH N=40 (below crossover, accuracy story) and N=80 (above it,
where the loop can plausibly beat the solver on cost AND accuracy).

    M3D_LP_N   fine grid   (40 or 80)
    M3D_LP_M   coarse grid for the defect
    M3D_LP_SEG number of closed-loop segments

Gates:
  L1  the loop does not diverge over the full horizon
  L2  DEGENERACY GUARD: final error must stay ABOVE the estimator residual.
      Below it the loop is reconstructing truth, not correcting.
  L3  the trust gate declines rather than harming when the estimate is poor
  L4  WALL-CLOCK: loop vs full solver vs uncorrected ROM, whole pipeline
"""
...
def parent_main(precision="fp64", jvp="analytic", deriv_rel=1e-5,
    ...
    print("=" * 100)
    print("M3D TEST X-v3 — DUAL GPU LAUNCHER  [alpha grid to 8, crc32 seed]")
    print("=" * 100)
    _banner("launcher")
...
def worker_main(physical_gpu: int, precision="fp64", jvp="analytic",
    ...
    LP_N_GRID = int(os.environ.get("M3D_LP_N", 40))
    LP_M      = int(os.environ.get("M3D_LP_M", 28))
    LP_SEG    = int(os.environ.get("M3D_LP_SEG", 10))
    LP_GATE   = float(os.environ.get("M3D_LP_GATE", 1.02))
    LP_HORIZON= float(os.environ.get("M3D_LP_HORIZON", 0.15))
    LP_FLOOR  = float(os.environ.get("M3D_LP_FLOOR", 1.1e-4))

    def _lp_sel(n_, m_):
        h = m_//2
        return torch.tensor(list(range(0, h+1)) + list(range(n_-h+1, n_)),
                            dtype=torch.long)

    def lp_restrict(U, n_, m_):
        i = _lp_sel(n_, m_).to(U.device)
        return (U.index_select(1,i).index_select(2,i).index_select(3,i)
                * (float(m_)/n_)**3)

    def lp_prolong(U, m_, n_):
        i = _lp_sel(n_, m_).to(U.device)
        o = torch.zeros((3,n_,n_,n_), dtype=U.dtype, device=U.device)
        o[:, i[:,None,None], i[None,:,None], i[None,None,:]] = U*(float(n_)/m_)**3
        return o
    ...
    assert max(NOISE_DELTAS_SCALE_X) == 7.00, "stale scale grid — wrong file"
    ...
    POINT_FILE = SAVE_DIR_X / f"m3d_LOOP3D_{tag}_gpu{physical_gpu}_interventions_partial.csv"
    CASE_FILE = SAVE_DIR_X / f"m3d_LOOP3D_{tag}_gpu{physical_gpu}_cases_partial.csv"
    SWEEP_FILE = SAVE_DIR_X / f"m3d_LOOP3D_{tag}_gpu{physical_gpu}_sweep_partial.csv"
    ...
    def star2_segment_pair_X(U1_start, U2_start, env):
        """ONE anchored ROM segment. Same primitive the baseline loop uses:
        fresh anchor, SEGMENT_STEPS_X of rk4_M, reconstruct."""
        model, _ = initialize_star2_N(U1_start, U2_start, env)
        for _ in range(SEGMENT_STEPS_X):
            rk4_M(model, DTAU_X, env)
        return reconstruct_M(model, env)
    ...
    for topology_X, nu_mult_X in CASES_THIS_GPU:
        ...
        ENV32_X = make_env_D(LP_N_GRID, L=8.0)
        ...
        ENV64_X = make_env_D(LP_N_GRID, L=8.0)
        ...
        # extend PAST the last intervention by the forecast horizon, so the
        # recurrence itself supplies b_hat with no extra machinery
        ...
        # Runs BEFORE the per-window interventions, on the same case. No truth
        # is read at decision time; truth is used only to score afterwards.
        import time as _t
        ENVC_X = make_env_D(LP_M, L=8.0) if LP_M < LP_N_GRID else ENV32_X

        def _coarse_phi(A1, A2):
            if LP_M >= LP_N_GRID:
                return phi_X(A1, A2, ENV32_X, nu_X)
            c1 = lp_restrict(A1, LP_N_GRID, LP_M)
            c2 = lp_restrict(A2, LP_N_GRID, LP_M)
            d1, d2 = phi_X(c1, c2, ENVC_X, nu_X)
            return (lp_prolong(d1, LP_M, LP_N_GRID),
                    lp_prolong(d2, LP_M, LP_N_GRID))

        _u1, _u2 = U10_X.clone(), U20_X.clone()
        # A: full solver (also the truth for scoring)
        _t0 = _t.time(); _T = [( _u1.clone(), _u2.clone() )]
        _a1, _a2 = _u1.clone(), _u2.clone()
        for _ in range(LP_SEG):
            _a1, _a2 = phi_X(_a1, _a2, ENV32_X, nu_X); _T.append((_a1.clone(), _a2.clone()))
        _t_full = _t.time() - _t0
        # B: uncorrected ROM
        _t0 = _t.time(); _R = [( _u1.clone(), _u2.clone() )]
        _b1, _b2 = _u1.clone(), _u2.clone()
        for _ in range(LP_SEG):
            _b1, _b2 = star2_segment_pair_X(_b1, _b2, ENV32_X)
            _R.append((_b1.clone(), _b2.clone()))
        _t_rom = _t.time() - _t0
        # D: closed loop
        _t0 = _t.time()
        _c1, _c2 = _u1.clone(), _u2.clone()
        _E1 = torch.zeros_like(_c1); _E2 = torch.zeros_like(_c2)
        _fired = _skipped = 0
        for _k in range(LP_SEG):
            _P1, _P2 = _coarse_phi(_c1, _c2)
            _N1, _N2 = star2_segment_pair_X(_c1, _c2, ENV32_X)
            _e1, _e2 = _P1 - _N1, _P2 - _N2
            if pair_norm_X(_E1, _E2) < 1e-30:
                _E1, _E2 = _e1.clone(), _e2.clone()
            else:
                _ne = max(pair_norm_X(_E1, _E2), 1e-30)
                _ep = min(max(deriv_rel*pair_norm_X(_c1,_c2)/_ne, 1e-6), 5e-2)
                _Pp1,_Pp2 = _coarse_phi(_c1+_ep*_E1, _c2+_ep*_E2)
                _Mm1,_Mm2 = _coarse_phi(_c1-_ep*_E1, _c2-_ep*_E2)
                _E1 = _e1 + (_Pp1-_Mm1)/(2*_ep) + 0.5*(_Pp1+_Mm1-2*_P1)/(_ep*_ep)
                _E2 = _e2 + (_Pp2-_Mm2)/(2*_ep) + 0.5*(_Pp2+_Mm2-2*_P2)/(_ep*_ep)
            _ph1,_ph2,_qh1,_qh2 = split_PQ_X(_E1, _E2, target_ids_X)
            _rel = pair_norm_X(_qh1,_qh2)/max(pair_norm_X(_N1,_N2), 1e-30)
            if (not np.isfinite(_rel)) or _rel > LP_HORIZON:
                _c1, _c2 = _N1, _N2; _skipped += 1; continue
            _F1,_F2 = star2_segment_pair_X(_N1, _N2, ENV32_X)
            _cands = {("none",0.0): (torch.zeros_like(_N1), torch.zeros_like(_N2))}
            for _al in (0.5,1.0,1.5,2.0):
                _cands[("Q",_al)] = (_al*_qh1.to(CDT_X), _al*_qh2.to(CDT_X))
                _cands[("P",_al)] = (_al*_ph1.to(CDT_X), _al*_ph2.to(CDT_X))
            _pr = {}
            for _kk,(_d1,_d2) in _cands.items():
                _g1,_g2 = star2_segment_pair_X(_N1+_d1, _N2+_d2, ENV32_X)
                _pr[_kk] = pack_modes_D(_g1-_F1, _g2-_F2, target_ids_X)
            _bh1,_bh2,_,_ = split_PQ_X(_E1, _E2, target_ids_X)
            _bh = -pack_modes_D(_bh1.to(CDT_X), _bh2.to(CDT_X), target_ids_X)
            _pk = min(_cands, key=lambda k_: float(
                torch.linalg.vector_norm(_bh + _pr[k_]).real))
            _Gh = float(torch.linalg.vector_norm(_bh).real)/max(float(
                torch.linalg.vector_norm(_bh + _pr[_pk]).real), 1e-30)
            if _Gh > LP_GATE and _pk[0] != "none":
                _d1,_d2 = _cands[_pk]
                _c1, _c2 = _N1+_d1, _N2+_d2; _fired += 1
                _E1 = _E1 - _d1; _E2 = _E2 - _d2
            else:
                _c1, _c2 = _N1, _N2; _skipped += 1
        _t_loop = _t.time() - _t0
        _e_rom  = target_error_X(_R[-1][0], _R[-1][1], _T[-1][0], _T[-1][1], target_ids_X)
        _e_loop = target_error_X(_c1, _c2, _T[-1][0], _T[-1][1], target_ids_X)
        LOOP_ROWS_X.append(dict(topology=topology_X, nu_mult=nu_mult_X,
            N=LP_N_GRID, M=LP_M, segments=LP_SEG,
            e_rom=_e_rom, e_loop=_e_loop, fired=_fired, skipped=_skipped,
            t_full=_t_full, t_rom=_t_rom, t_loop=_t_loop,
            degenerate=bool(_e_loop < LP_FLOOR)))
        print(f"[GPU{physical_gpu}] LOOP {case_X}: fired {_fired}/{LP_SEG}  "
              f"ROM {100*_e_rom:.3f}%  LOOP {100*_e_loop:.3f}%  "
              f"cost {_t_loop/max(_t_full,1e-9):.2f}x solver", flush=True)
        # ==========================================================
        ...
        for tau_int_X in INTERVENTION_TAUS_X:
            ...
            # ---------------- ADAPTIVE SELECTION: no truth used ----------
            EHP1_64, EHP2_64, _, _ = split_PQ_X(EH1_64, EH2_64, target_ids_X)
            EHP1, EHP2 = EHP1_64.to(CDT_X), EHP2_64.to(CDT_X)
            EHF1, EHF2 = EH1_64.to(CDT_X), EH2_64.to(CDT_X)
            # full-state is NOT a candidate. With this estimator (W hidden-Q
            # residual ~1.1e-4) V + e_hat reproduces truth to estimator
            # precision, so "correcting" with it is STATE REPLACEMENT, not
            # model correction: the first AD3D run gave 87x with a corrected
            # P-error of 5.6e-5, i.e. BELOW the estimator's own noise floor.
            # It is kept as a DIAGNOSTIC column only.
            AD_ALPHAS = [0.5, 1.0, 1.3, 1.5, 2.0, 2.5, 3.0]
            cands = {("none", 0.0): (torch.zeros_like(V1), torch.zeros_like(V2))}
            for _a in AD_ALPHAS:
                cands[("Q", _a)] = (_a*EHQ1, _a*EHQ2)
                cands[("P", _a)] = (_a*EHP1, _a*EHP2)
            FB1p, FB2p = star2_forecast_X(V1, V2, ENV32_X)
            probe = {}
            for _k, (_c1, _c2) in cands.items():
                _f1, _f2 = star2_forecast_X(V1+_c1, V2+_c2, ENV32_X)
                probe[_k] = pack_modes_D(_f1-FB1p, _f2-FB2p, target_ids_X)
            # b_hat: read the recurrence FORWARD at the forecast horizon.
            # e_hat estimates (truth - V); b is (forecast - truth), hence the sign.
            _bi = min(idx + FORECAST_SEGMENTS_X, len(EHAT_TRAJ_X) - 1)
            _BE1, _BE2 = EHAT_TRAJ_X[_bi]
            _bh1, _bh2, _, _ = split_PQ_X(_BE1, _BE2, target_ids_X)
            bh = -pack_modes_D(_bh1.to(CDT_X), _bh2.to(CDT_X), target_ids_X)
            pick = min(cands, key=lambda k_: float(
                torch.linalg.vector_norm(bh + probe[k_]).real))
            AD1, AD2 = cands[pick]
            G_hat_ad = float(torch.linalg.vector_norm(bh).real) / max(float(
                torch.linalg.vector_norm(bh + probe[pick]).real), 1e-30)
            # -------------------------------------------------------------
            ...
            # --- the decomposition: b = baseline observable error,
            #     Delta = the oracle's observable effect on P ---
            bv = pack_modes_D(FB1 - TF1, FB2 - TF2, target_ids_X)
            dv = pack_modes_D(FO1 - FB1, FO2 - FB2, target_ids_X)
            nb = float(torch.linalg.vector_norm(bv).real)
            nd = float(torch.linalg.vector_norm(dv).real)
            if nb > 1e-300 and nd > 1e-300:
                lam = nd / nb
                cth = float(torch.real(torch.vdot(bv, dv)).cpu()) / (nb * nd)
                pred = 1.0 / math.sqrt(max(1.0 + 2*lam*cth + lam*lam, 1e-300))
            else:
                lam = cth = float("nan"); pred = float("nan")

            # adaptive arm, and the fixed families it must beat
            _FA1, _FA2 = star2_forecast_X(V1+AD1, V2+AD2, ENV32_X)
            ad_err = target_error_X(_FA1, _FA2, TF1, TF2, target_ids_X)
            _FP1, _FP2 = star2_forecast_X(V1+EHP1, V2+EHP2, ENV32_X)
            p_err = target_error_X(_FP1, _FP2, TF1, TF2, target_ids_X)
            _FF1, _FF2 = star2_forecast_X(V1+EHF1, V2+EHF2, ENV32_X)
            f_err = target_error_X(_FF1, _FF2, TF1, TF2, target_ids_X)
            # DEGENERACY GUARD: if a corrected error lands at or below the
            # estimator's own residual, it is not a correction — flag it.
            W_RESIDUAL_FLOOR = 1.1e-4
            ad_degenerate = bool(ad_err < W_RESIDUAL_FLOOR)
            full_degenerate = bool(f_err < W_RESIDUAL_FLOOR)
            _xa = pack_modes_D(V1+AD1, V2+AD2, target_ids_X)
            ad_mismatch = float((torch.linalg.vector_norm(_xa-xb)
            ...
            row = {
                ...
                "pick": pick[0], "pick_alpha": pick[1], "G_hat_ad": G_hat_ad,
                "gain_adaptive": base_err/max(ad_err,1e-30),
                "gain_Ponly": base_err/max(p_err,1e-30),
                "gain_full": base_err/max(f_err,1e-30),
                "ad_mismatch": ad_mismatch,
                "ad_degenerate": ad_degenerate, "full_degenerate": full_degenerate,
                "ad_P_error": ad_err, "full_P_error": f_err,
                "lam": lam, "cos_theta": cth,
                "gain_predicted": pred, "gain_oracle_obs": base_err/max(ora_err,1e-30),
                "identity_err": abs(pred - base_err/max(ora_err,1e-30)),
    ...
    if LOOP_ROWS_X:
        pd.DataFrame(LOOP_ROWS_X).to_csv(
            SAVE_DIR_X / f"m3d_LOOP3D_{tag}_gpu{physical_gpu}_loop.csv", index=False)
...
def finalize_outputs(tag="fp64_analytic", noise_sweep=True,
    ...
    pfiles = [SAVE_DIR / f"m3d_LOOP3D_{tag}_gpu{g}_interventions_partial.csv" for g in (0, 1)]
    cfiles = [SAVE_DIR / f"m3d_LOOP3D_{tag}_gpu{g}_cases_partial.csv" for g in (0, 1)]
    sfiles = [SAVE_DIR / f"m3d_LOOP3D_{tag}_gpu{g}_sweep_partial.csv" for g in (0, 1)]
    ...
    RESULTS_X.to_csv(SAVE_DIR / f"m3d_LOOP3D_{tag}_causal_interventions.csv", index=False)
    CASES_X.to_csv(SAVE_DIR / f"m3d_LOOP3D_{tag}_case_summary.csv", index=False)
    ...
    _lf = sorted(SAVE_DIR_PARENT.glob(f"m3d_LOOP3D_{tag}_gpu*_loop.csv")) \
          if "SAVE_DIR_PARENT" in dir() else \
          sorted(Path("/kaggle/working/M3D").glob(f"m3d_LOOP3D_{tag}_gpu*_loop.csv"))
    if _lf:
        import numpy as _np
        LP = pd.concat([pd.read_csv(f) for f in _lf], ignore_index=True)
        print("\n" + "="*100)
        print(f"LOOP3D — AUTONOMOUS CLOSED LOOP   N={int(LP.N.iloc[0])} "
              f"M={int(LP.M.iloc[0])}  {int(LP.segments.iloc[0])} segments")
        print("="*100)
        print(f"\nL1 no divergence: "
              f"{int(_np.isfinite(LP.e_loop).sum())}/{len(LP)} cases finite"
              f"  -> {'PASS' if _np.all(_np.isfinite(LP.e_loop)) else 'FAIL'}")
        _nd = int(LP.degenerate.sum())
        print(f"L2 degeneracy guard: {_nd}/{len(LP)} below the estimator residual"
              f"  -> {'PASS' if _nd == 0 else 'FAIL — reconstructing truth'}")
        print(f"\nL3 accuracy at the final segment")
        print(f"   {'method':<26}{'median':>11}{'p90':>11}")
        for _nm,_c in (("uncorrected ROM","e_rom"),("CLOSED LOOP","e_loop")):
            _v = LP[_c].values
            print(f"   {_nm:<26}{100*_np.median(_v):>10.3f}%"
                  f"{100*_np.percentile(_v,90):>10.3f}%")
        _imp = _np.median(LP.e_rom.values)/max(_np.median(LP.e_loop.values),1e-30)
        print(f"   improvement over uncorrected: {_imp:.2f}x")
        print(f"   trust gate fired {LP.fired.median():.0f}/"
              f"{int(LP.segments.iloc[0])} segments, declined {LP.skipped.median():.0f}")
        print(f"\nL4 WALL-CLOCK, whole pipeline")
        print(f"   {'component':<26}{'total s':>10}{'vs solver':>12}")
        _tf = LP.t_full.sum()
        for _nm,_c in (("full solver","t_full"),("uncorrected ROM","t_rom"),
                       ("CLOSED LOOP","t_loop")):
            print(f"   {_nm:<26}{LP[_c].sum():>10.1f}{LP[_c].sum()/max(_tf,1e-9):>11.2f}x")
        _r = LP.t_loop.sum()/max(_tf,1e-9)
        print(f"\n   -> the loop costs {_r:.2f}x the full solver at N="
              f"{int(LP.N.iloc[0])}")
        print(f"      (benchmark reference: estimator alone was 3.06x at N=40,"
              f" 0.71x at N=64, 0.34x at N=80)")

    if "pick" in RESULTS_X.columns:
        import numpy as _np
        AD = RESULTS_X.dropna(subset=["gain_adaptive"])
        print("\n" + "="*100)
        print("AD3D — ADAPTIVE CORRECTION, 3D NAVIER-STOKES  (full-state EXCLUDED)")
        print("="*100)
        print(f"\n  {'strategy':<34}{'median':>10}{'p10':>10}{'win%':>8}{'worst':>10}")
        for _nm,_c in (("uncorrected",None),
                       ("Q-only, alpha=1 (X-v3)","estimated_gain"),
                       ("P-only, alpha=1","gain_Ponly"),
                       ("ADAPTIVE (ours)","gain_adaptive")):
            _v = _np.ones(len(AD)) if _c is None else AD[_c].values
            print(f"  {_nm:<34}{_np.median(_v):>10.4f}{_np.percentile(_v,10):>10.4f}"
                  f"{100*_np.mean(_v>=1.0):>7.0f}%{_np.min(_v):>10.4f}")
        _best = max(_np.median(AD[c].values) for c in
                    ("estimated_gain","gain_Ponly"))
        _ad = float(_np.median(AD.gain_adaptive.values))
        print(f"\nT1 beats every fixed strategy: {_ad:.4f} vs {_best:.4f}"
              f"  -> {'PASS' if _ad > _best else 'FAIL'}")
        print(f"T2 beats published X-v3 (1.6510): {_ad:.4f}"
              f"  -> {'PASS' if _ad > 1.6510 else 'FAIL'}")
        _q = AD[AD.pick == "Q"]
        _mm = float(_q.ad_mismatch.max()) if len(_q) else 0.0
        print(f"T3 dP on Q-picks: {_mm:.3e}"
              f"  -> {'PASS' if _mm < 1e-12 else 'FAIL'}")
        _nd = int(AD.ad_degenerate.sum()) if "ad_degenerate" in AD else 0
        print(f"\nT5 degeneracy guard: {_nd}/{len(AD)} adaptive windows at or below"
              f" the estimator residual (1.1e-4)"
              f"  -> {'PASS' if _nd == 0 else 'FAIL — state replacement'}")
        if "full_P_error" in AD:
            _fd = int(AD.full_degenerate.sum())
            print(f"    DIAGNOSTIC, full-state (excluded as a candidate):")
            print(f"    degenerate on {_fd}/{len(AD)} windows, median corrected"
                  f" P-error {AD.full_P_error.median():.3e} vs residual 1.1e-04,")
            print(f"    nominal gain {AD.gain_full.median():.1f}x — that is state"
                  f" replacement, not correction.")
        print(f"\nT4 picks")
        for _k,_g in AD.groupby("pick"):
            print(f"    {_k:<6} {len(_g):>3} win   alpha {_g.pick_alpha.median():.2f}"
                  f"   lam {_g.lam.median():.4f}   gain {_g.gain_adaptive.median():.4f}")
        print(f"    observable preserved on {len(_q)}/{len(AD)} windows")
        print(f"    Spearman(G_hat, realised) "
              f"{AD[['G_hat_ad','gain_adaptive']].corr(method='spearman').iloc[0,1]:+.4f}")

    if "lam" in RESULTS_X.columns:
        L = RESULTS_X.dropna(subset=["lam"])
        print("\n" + "="*96)
        print("LAMBDA / COS DECOMPOSITION  —  gain = 1/sqrt(1 + 2*lam*cos + lam^2)")
        print("="*96)
        print(f"  L0 identity max error = {L.identity_err.max():.3e}"
              f"  -> {'PASS' if L.identity_err.max() < 1e-9 else 'FAIL'}")
        ml, mc = float(L.lam.median()), float(L.cos_theta.median())
        print(f"\n  L1 median lambda = {ml:.4f}  (KS: 0.048-0.134; need > 0.35)"
              f"  -> {'PASS' if ml > 0.35 else 'FAIL'}")
        print(f"  L2 median cos    = {mc:+.4f}  (KS: -0.64 to +0.12; need < -0.6)"
              f"  -> {'PASS' if mc < -0.6 else 'FAIL'}")
        print(f"\n  L3 per case")
        print(f"  {'case':<26}{'lam':>9}{'cos':>9}{'pred gain':>11}{'oracle':>9}")
        for c, g in L.groupby("case"):
            print(f"  {c:<26}{g.lam.median():>9.4f}{g.cos_theta.median():>+9.4f}"
                  f"{g.gain_predicted.median():>11.4f}{g.gain_oracle_obs.median():>9.4f}")
        print(f"\n  CONTRAST on one axis")
        print(f"  {'system':<26}{'lam':>9}{'cos':>9}{'gain':>9}")
        print(f"  {'Kuramoto-Sivashinsky':<26}{0.0891:>9.4f}{-0.4118:>+9.4f}{1.0350:>9.4f}")
        print(f"  {'Navier-Stokes':<26}{ml:>9.4f}{mc:>+9.4f}"
              f"{float(L.gain_oracle_obs.median()):>9.4f}")
        print("\n  If NS lambda is ~5-10x KS's, the magnitude gap is a LAMBDA gap:")
        print("  how much of the observable error is CAUSED by hidden leakage.")
    ...
    if noise_sweep and all(f.exists() for f in sfiles):
        ...
        SW.to_csv(SAVE_DIR / f"m3d_LOOP3D_{tag}_estimator_sweep.csv", index=False)

        print("\n" + "=" * 156)
        print("M3D TEST X-v3 — ESTIMATOR-TOLERANCE SWEEP")
        print("=" * 156)
    ...
    with open(SAVE_DIR / f"m3d_LOOP3D_{tag}_COMPLETE.txt", "w") as f:
        f.write(
    ...
    print("\nOutputs:")
    print(SAVE_DIR / f"m3d_LOOP3D_{tag}_causal_interventions.csv")
    print(SAVE_DIR / f"m3d_LOOP3D_{tag}_case_summary.csv")
    if noise_sweep:
        print(SAVE_DIR / f"m3d_LOOP3D_{tag}_estimator_sweep.csv")
    print(SAVE_DIR / f"m3d_LOOP3D_{tag}_COMPLETE.txt")

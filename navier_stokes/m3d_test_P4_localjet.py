# ============================================================================
# M3D TEST P4 — IS IT THE LOCAL TRAJECTORY JET?
# ============================================================================
# Three explanations have now been tested for why hidden-state correction is
# ~9x weaker on POD-Galerkin (1.04x) than on STAR2 (1.65x):
#
#   P2  reservoir ratio via rank sweep     FAILED (sign flipped, no leverage)
#   P3  containment of the observable set  REAL AND CAUSAL, 8/8 monotone,
#                                          but explains only 16% of the gap
#
# 84% remains. This tests the last big structural difference.
#
#   STAR2 subspace :  P + q0 + q1*   where q0 = normalise(Q F(V)) and
#                     q1* = normalise(R1 DF(V) F(V)) are REBUILT AT EVERY
#                     ANCHOR from the local trajectory jet.
#   POD subspace   :  one global basis, frozen for all time.
#
# Both jet directions are computable from the ROM state alone -- no truth --
# so adding them to a POD basis is a legitimate ROM, not a cheat.
#
# FIVE VARIANTS on identical unseen test trajectories:
#   A  POD only, frozen                       (the P1/P3 baseline, ~1.04x)
#   B  POD + all of P                         (P3 endpoint, ~1.11x)
#   C  POD + all of P + q0                    first jet direction
#   D  POD + all of P + q0 + q1*              both jet directions
#   E  P + q0 + q1* only                      STAR2's subspace exactly
#
# E is the harness control: if it does NOT reproduce STAR2's ~1.65x, the
# comparison is invalid and nothing else in this test can be trusted.
#
# Preregistered gates:
#   J0  HARNESS VALID: variant E median gain within 15% of 1.651
#   J1  the jet matters: D beats B on >= 80% of trajectories, paired
#   J2  ATTRIBUTION: fraction of the log-gap from A to E explained by adding
#       the jet (B->D) versus by containment (A->B)
#   J3  DESCRIPTIVE: is q0 alone enough, or is q1* required? (C vs D)
# ============================================================================
...
N    = int(os.environ.get("M3D_P4_N", 40))
NU   = float(os.environ.get("M3D_P4_NU", 2.5e-3))
N_TRAIN = int(os.environ.get("M3D_P4_NTRAIN", 12))
N_VAL   = int(os.environ.get("M3D_P4_NVAL", 6))
N_TEST  = int(os.environ.get("M3D_P4_NTEST", 20))
N_SEG   = 12
TAUS    = [0.60, 0.90, 1.20]
FORECAST_SEGS, DERIV_REL = 2, 1e-5
RANKS   = [int(r) for r in os.environ.get("M3D_P4_RANKS", "10").split(",")]
VARIANTS = os.environ.get("M3D_P4_VARIANTS", "A,B,C,D,E").split(",")
    ...
    for ti in range(N_TEST):
        ...
        try:
            ...
            sub=[r for r in rows if r["traj"]==ti and r["variant"]==VAR
                 and r["status"]=="ok"]
            if sub:
                print(f"    traj {ti:>3}: dim={dim:>3}  "
                      f"R={np.median([r['reservoir'] for r in sub]):5.2f}  "
...
ok = df[df.status == "ok"]
print("\n" + "="*104)
print("M3D TEST P4 — IS THE GAP THE LOCAL TRAJECTORY JET?")
print("="*104)
print(f"\n  {'var':>4}  {'subspace':<32}{'dim':>5}{'median R':>10}"
      f"{'median base':>13}{'median gain':>13}{'reduction':>11}{'win':>8}")
tab = {}
for v in [x for x in VARIANTS if x in set(ok.variant)]:
    s_ = ok[ok.variant == v]; pr = s_.groupby("traj")
    g = float(pr.gain.median().median())
    tab[v] = dict(g=g, R=float(pr.reservoir.median().median()),
                  b=float(pr.baseline_P_error.median().median()),
                  d=int(s_.subspace_dim.median()), w=float(s_.beats.mean()))
    print(f"  {v:>4}  {VDESC[v]:<32}{tab[v]['d']:>5}{tab[v]['R']:>10.2f}"
          f"{100*tab[v]['b']:>12.2f}%{g:>13.4f}{100*(1-1/g):>10.2f}%"
          f"{100*tab[v]['w']:>7.0f}%")

if "E" in tab:
    dev = abs(tab["E"]["g"] - 1.651)/1.651
    print(f"\nJ0 HARNESS CONTROL: variant E = {tab['E']['g']:.4f}x vs STAR2 1.6510x"
          f"  ({100*dev:.1f}% off)  -> {'PASS' if dev < 0.15 else 'FAIL'}")
    if dev >= 0.15:
        print("   The harness does not reproduce STAR2. Nothing below is valid.")

def paired(v1, v2):
    a = ok[ok.variant==v1].groupby("traj").gain.median()
    b = ok[ok.variant==v2].groupby("traj").gain.median()
    c = a.index.intersection(b.index)
    return int((b[c] > a[c]).sum()), len(c), float(a[c].median()), float(b[c].median())
if "B" in tab and "D" in tab:
    n, m, ga, gb = paired("B", "D")
    print(f"\nJ1 adding the local jet: D beats B on {n}/{m} trajectories, paired"
          f"  -> {'PASS' if m and n >= 0.8*m else 'FAIL'}")
    print(f"   median {ga:.4f}x -> {gb:.4f}x")

if all(k in tab for k in ("A","B","D","E")):
    r = {k: 1-1/tab[k]["g"] for k in ("A","B","D","E")}
    tot = np.log(r["E"]/r["A"])
    print(f"\nJ2 ATTRIBUTION of the A -> E gap ({100*r['A']:.2f}% -> {100*r['E']:.2f}% reduction)")
    for lbl, x, y in (("containment  (A->B)","A","B"),
                      ("local jet    (B->D)","B","D"),
                      ("dropping POD (D->E)","D","E")):
        f = np.log(r[y]/r[x])/tot if tot else float("nan")
        print(f"   {lbl}: {100*r[x]:>6.2f}% -> {100*r[y]:>6.2f}%   "
              f"{100*f:>6.1f}% of the log-gap")

if "C" in tab and "D" in tab:
    print(f"\nJ3 is q1* needed, or is q0 enough?")
    print(f"   C (q0 only)      {tab['C']['g']:.4f}x")
    print(f"   D (q0 and q1*)   {tab['D']['g']:.4f}x")
    print(f"   -> {'q1* adds real value' if tab['D']['g'] > 1.02*tab['C']['g'] else 'q0 carries most of it'}")
print("\nsaved:", OUT)

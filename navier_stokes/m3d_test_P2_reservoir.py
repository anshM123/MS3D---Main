# ============================================================================
# M3D TEST P2 — WHAT GOVERNS THE SIZE OF THE EFFECT?
# ============================================================================
# P1 answered the kill gate: hidden-state correction reproduces on POD-Galerkin
# under an honest train/test protocol (20/20 trajectories, 60/60 windows,
# oracle benefit captured 1.0004). But the effect was 9.5x weaker than on
# STAR2 -- 4.2% error reduction against 39.4%.
#
# The estimator is NOT the limitation: it captures 100.04% of what the oracle
# achieves. The oracle itself only buys 4%. So there is simply less exploitable
# hidden error in this ROM.
#
# HYPOTHESIS. STAR2 is built as Pi = P + q0 + q1*, so P is CONTAINED in the ROM
# subspace by construction. A global POD basis is not. If a ROM cannot
# represent its own observables exactly, much of its error lands in P rather
# than Q -- and there is less hidden error to reach for.
#
#   PREDICTION:  gain tracks the reservoir ratio  R = ||Qe|| / ||Pe||,
#                NOT the size of the error.
#
# STAR2 measured R ~ 16-23 and got 1.65x. If POD sits near R ~ 1 and gets
# 1.04x, and if sweeping the POD rank moves R and gain together, the
# discrepancy becomes a quantitative design principle rather than an anomaly.
#
# This sweeps POD rank, and for each rank measures on the SAME unseen test
# trajectories:
#     R = ||Qe||/||Pe||          the reservoir ratio
#     containment  ||P - P_S P||  how much of the observable set the ROM
#                                 subspace actually contains
#     gain, win rate, oracle capture
#
# Preregistered gates:
#   H1  R varies by >= 3x across the rank sweep (otherwise the sweep has no
#       leverage and the hypothesis cannot be tested)
#   H2  Spearman(R, median gain) > 0.6 across rank x trajectory
#   H3  gain correlates BETTER with R than with baseline error magnitude
#       (this is the discriminating comparison)
#   H4  DESCRIPTIVE: containment vs R vs gain, and where STAR2 sits on the
#       same axes
# ============================================================================
...
N    = int(os.environ.get("M3D_P2_N", 40))
NU   = float(os.environ.get("M3D_P2_NU", 2.5e-3))
N_TRAIN = int(os.environ.get("M3D_P2_NTRAIN", 12))
N_VAL   = int(os.environ.get("M3D_P2_NVAL", 6))
N_TEST  = int(os.environ.get("M3D_P2_NTEST", 20))
N_SEG   = 12
TAUS    = [0.60, 0.90, 1.20]
FORECAST_SEGS, DERIV_REL = 2, 1e-5
RANKS   = [int(r) for r in os.environ.get("M3D_P2_RANKS", "6,10,16,24").split(",")]
...
SWEEP = [r for r in RANKS if 0.002 <= val[r] <= 0.40]
if len(SWEEP) < 2:
    SWEEP = RANKS
print(f"  -> sweeping ranks {SWEEP} (P2 measures ALL of them, not one)", flush=True)
...
done = {(int(r["traj"]), int(r["rank"])) for r in rows}
CONT = {}
for r in SWEEP:
    M1r, M2r, _ = BASES[r]
    CONT[r] = containment(M1r, M2r, TIDS, env)
    print(f"  rank {r:>3}: P-containment = {CONT[r]:.4f}", flush=True)

for RANK in SWEEP:
    M1, M2, CAP = BASES[RANK]
    print(f"\n  === rank {RANK} (containment {CONT[RANK]:.4f}) ===", flush=True)
    for ti in range(N_TEST):
        if (ti, RANK) in done: continue
        try:
            ...
            for tau in TAUS:
                ...
                base = dict(traj=ti, tau=tau, rank=RANK, containment=CONT[RANK],
            ...
            sub=[r for r in rows if r["traj"]==ti and r["rank"]==RANK and r["status"]=="ok"]
            if sub:
...
ok = df[df.status == "ok"]
print("\n" + "="*100)
print("M3D TEST P2 — DOES THE RESERVOIR RATIO GOVERN THE EFFECT SIZE?")
print("="*100)
print(f"\n  {'rank':>5}{'contain':>10}{'median R':>11}{'median base':>13}"
      f"{'median gain':>13}{'win rate':>10}{'oracle cap':>12}")
per_rank = {}
for r in sorted(ok["rank"].unique()):
    s = ok[ok["rank"] == r]
    pr = s.groupby("traj")
    R = float(pr.reservoir.median().median())
    b = float(pr.baseline_P_error.median().median())
    g = float(pr.gain.median().median())
    w = float(s.beats.mean())
    cap = float(((s.baseline_P_error-s.est_P_error)
                 /(s.baseline_P_error-s.oracle_P_error).replace(0,np.nan)).median())
    per_rank[r] = (R, b, g)
    print(f"  {int(r):>5}{s.containment.iloc[0]:>10.4f}{R:>11.3f}{100*b:>12.2f}%"
          f"{g:>13.4f}{100*w:>9.1f}%{cap:>12.4f}")

Rs = [v[0] for v in per_rank.values()]
print(f"\nH1 reservoir ratio varies across the sweep: "
      f"{min(Rs):.3f} to {max(Rs):.3f} = {max(Rs)/max(min(Rs),1e-9):.2f}x"
      f"  -> {'PASS' if max(Rs)/max(min(Rs),1e-9) >= 3 else 'FAIL (no leverage)'}")

def spearman(x, y):
    rx = np.argsort(np.argsort(x)); ry = np.argsort(np.argsort(y))
    return float(np.corrcoef(rx, ry)[0, 1])
pt = ok.groupby(["rank","traj"]).median(numeric_only=True).reset_index()
sR = spearman(pt.reservoir.values, pt.gain.values)
sB = spearman(pt.baseline_P_error.values, pt.gain.values)
print(f"\nH2 Spearman(reservoir R, gain) = {sR:+.4f}   n={len(pt)}"
      f"  -> {'PASS' if sR > 0.6 else 'FAIL'}")
print(f"\nH3 the discriminating comparison")
print(f"     Spearman(R, gain)              = {sR:+.4f}")
print(f"     Spearman(baseline error, gain) = {sB:+.4f}")
_v3 = ("PASS: gain is governed by WHERE the error is, not HOW BIG it is"
       if abs(sR) > abs(sB) else
       "FAIL: error magnitude explains gain at least as well")
print(f"  -> {_v3}")

print("\nH4 where the two ROM families sit on the same axes")
print(f"  {'model':<26}{'containment':>13}{'reservoir R':>13}{'gain':>10}")
for r in sorted(per_rank):
    R,b,g = per_rank[r]
    print(f"  {'POD-Galerkin rank '+str(int(r)):<26}"
          f"{ok[ok['rank']==r].containment.iloc[0]:>13.4f}{R:>13.3f}{g:>10.4f}")
print(f"  {'STAR2 (P contained)':<26}{1.0:>13.4f}{'16-23':>13}{1.6510:>10.4f}")
print("\n  If gain rises with R across the POD sweep and STAR2 sits at the top of")
print("  that same trend, the 9.5x discrepancy is explained by a single variable:")
print("  the fraction of accumulated error that lies outside the observable set.")
print("\nsaved:", OUT)

#!/usr/bin/env python3
"""
M3D TEST X-v3 — dual-T4 Kaggle runner with extended alpha sweep
...
BUILD_ID = "X3-2026-08-30-alpha8-crc32"
...
def worker_main(physical_gpu: int, precision="fp64", jvp="analytic",
    ...
    POINT_FILE = SAVE_DIR_X / f"m3d_X3_{tag}_gpu{physical_gpu}_interventions_partial.csv"
    CASE_FILE = SAVE_DIR_X / f"m3d_X3_{tag}_gpu{physical_gpu}_cases_partial.csv"
    SWEEP_FILE = SAVE_DIR_X / f"m3d_X3_{tag}_gpu{physical_gpu}_sweep_partial.csv"
...
def finalize_outputs(tag="fp64_analytic", noise_sweep=True,
    ...
    pfiles = [SAVE_DIR / f"m3d_X3_{tag}_gpu{g}_interventions_partial.csv" for g in (0, 1)]
    cfiles = [SAVE_DIR / f"m3d_X3_{tag}_gpu{g}_cases_partial.csv" for g in (0, 1)]
    sfiles = [SAVE_DIR / f"m3d_X3_{tag}_gpu{g}_sweep_partial.csv" for g in (0, 1)]
    ...
    RESULTS_X.to_csv(SAVE_DIR / f"m3d_X3_{tag}_causal_interventions.csv", index=False)
    CASES_X.to_csv(SAVE_DIR / f"m3d_X3_{tag}_case_summary.csv", index=False)
    ...
    if noise_sweep and all(f.exists() for f in sfiles):
        ...
        SW.to_csv(SAVE_DIR / f"m3d_X3_{tag}_estimator_sweep.csv", index=False)
    ...
    with open(SAVE_DIR / f"m3d_X3_{tag}_COMPLETE.txt", "w") as f:
        f.write(
    ...
    print("\nOutputs:")
    print(SAVE_DIR / f"m3d_X3_{tag}_causal_interventions.csv")
    print(SAVE_DIR / f"m3d_X3_{tag}_case_summary.csv")
    if noise_sweep:
        print(SAVE_DIR / f"m3d_X3_{tag}_estimator_sweep.csv")
    print(SAVE_DIR / f"m3d_X3_{tag}_COMPLETE.txt")

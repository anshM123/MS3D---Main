#!/usr/bin/env python3
"""
M3D TEST PSW — NS P-SWEEP: does observable stability set lambda?

Runs the 12 Test-X cases across two independent CUDA subprocesses.

WHY THIS EXISTS

On Kuramoto-Sivashinsky, holding the equation, resolution, ROM construction and
protocol fixed and changing ONLY the dynamical role of the observable set P:

  band            growth rate    lambda     cos     gain    p10
  unstable            +0.05      0.090   -0.616   1.055   1.013
  strongly damped    -15.49      1.045   -0.9993 14.300   8.261
  (replicated at L=200: damped gain 19.71, p10 11.49; 64 windows total,
   per-window identity |pred-obs|/obs median 1.97e-15)

Spearman(growth rate, lambda) = -0.900. When P is strongly damped, its own
error decays, so essentially ALL remaining observable error is hidden-driven:
lambda ~ 1 with cos ~ -1, and correcting the hidden state removes nearly all of
it. Where cos turns POSITIVE (near-neutral band) the correction HURTS and the
wrong-sign arm helps -- the law predicts the sign flip, not just the magnitude.

THIS TEST asks whether the same law holds in Navier-Stokes. The NS viscous
operator gives growth rate ~ -nu*k^2, so moving P to higher wavenumbers moves
it into strongly damped territory exactly as it did in KS.

PREREGISTERED PREDICTION (written before running):
  As the P band moves to higher |k|, the mean growth rate -nu*k^2 falls,
  and we predict lambda RISES toward ~1, cos theta approaches -1, and the
  gain rises well above the 1.651x measured on the standard 'mixed' projector.
  Spearman(growth rate, lambda) should be strongly negative.

If it holds, the effect-size law is confirmed on two independent equations with
a mechanism, and lambda becomes predictable from the linear operator restricted
to the observable set.

Gates:
  W0  identity holds per window: |predicted - observed|/observed < 1e-9
  W1  Spearman(mean growth rate over P, lambda) < -0.6
  W2  the most damped band beats the standard projector's 1.651x
  W3  DESCRIPTIVE: lambda, cos, gain per band, alongside the KS numbers
"""
...
def worker_main(physical_gpu: int, precision="fp64", jvp="analytic",
    ...
    POINT_FILE = SAVE_DIR_X / f"m3d_PSW_{tag}_gpu{physical_gpu}_interventions_partial.csv"
    CASE_FILE = SAVE_DIR_X / f"m3d_PSW_{tag}_gpu{physical_gpu}_cases_partial.csv"
    SWEEP_FILE = SAVE_DIR_X / f"m3d_PSW_{tag}_gpu{physical_gpu}_sweep_partial.csv"
    ...
    for topology_X, nu_mult_X in CASES_THIS_GPU:
        ...
        # ---- P-SWEEP: replace the standard projector with a band in |k| ----
        PSW_BAND = os.environ.get("M3D_PSW_BAND", "").strip()
        if PSW_BAND:
            klo, khi = (float(x) for x in PSW_BAND.split(","))
            _N = ENV32_X["K2"].shape[-1]
            _km = torch.sqrt(ENV32_X["K2"]).reshape(-1)
            _sel = torch.nonzero((_km >= klo) & (_km <= khi)).reshape(-1)
            # keep the same mode COUNT as the standard projector so band and
            # dimension are not confounded
            _want = int(target_ids_X.numel())
            if _sel.numel() > _want:
                _ord = torch.argsort(torch.abs(_km[_sel] - 0.5*(klo+khi)))
                _sel = _sel[_ord[:_want]]
            if _sel.numel() < 4:
                print(f"[GPU{physical_gpu}] band {PSW_BAND}: only "
                      f"{_sel.numel()} modes, skipping", flush=True)
                continue
            target_ids_X = _sel.to(target_ids_X.device).to(target_ids_X.dtype)
            model_X["proj"]["target_ids"] = target_ids_X
            _kb = _km[target_ids_X]
            PSW_KMEAN = float(_kb.mean().cpu())
            PSW_GROWTH = float(-(nu_X * (_kb**2)).mean().cpu())
            print(f"[GPU{physical_gpu}] P band |k| in [{klo},{khi}]: "
                  f"{target_ids_X.numel()} modes, mean|k|={PSW_KMEAN:.3f}, "
                  f"mean growth rate={PSW_GROWTH:+.5f}", flush=True)
        else:
            _km = torch.sqrt(ENV32_X["K2"]).reshape(-1)
            _kb = _km[target_ids_X]
            PSW_KMEAN = float(_kb.mean().cpu())
        ...
        for tau_int_X in INTERVENTION_TAUS_X:
            ...
            row = {
                ...
                "P_band": PSW_BAND if PSW_BAND else "standard",
                "P_kmean": PSW_KMEAN, "P_growth": PSW_GROWTH,
...
def finalize_outputs(tag="fp64_analytic", noise_sweep=True,
    ...
    pfiles = [SAVE_DIR / f"m3d_PSW_{tag}_gpu{g}_interventions_partial.csv" for g in (0, 1)]
    cfiles = [SAVE_DIR / f"m3d_PSW_{tag}_gpu{g}_cases_partial.csv" for g in (0, 1)]
    sfiles = [SAVE_DIR / f"m3d_PSW_{tag}_gpu{g}_sweep_partial.csv" for g in (0, 1)]
    ...
    RESULTS_X.to_csv(SAVE_DIR / f"m3d_PSW_{tag}_causal_interventions.csv", index=False)
    CASES_X.to_csv(SAVE_DIR / f"m3d_PSW_{tag}_case_summary.csv", index=False)
    ...
    if "P_band" in RESULTS_X.columns:
        L = RESULTS_X.dropna(subset=["lam"])
        print("\n" + "="*100)
        print("NS P-SWEEP  —  does observable stability set lambda, as it does in KS?")
        print("="*100)
        print(f"  {'band':<14}{'mean|k|':>9}{'growth':>11}{'n':>5}{'base%':>9}"
              f"{'lambda':>9}{'cos':>9}{'gain':>10}{'p10':>9}")
        for bd, g in L.groupby("P_band"):
            print(f"  {bd:<14}{g.P_kmean.iloc[0]:>9.3f}{g.P_growth.iloc[0]:>+11.5f}"
                  f"{len(g):>5}{100*g.baseline_P_error.median():>8.3f}%"
                  f"{g.lam.median():>9.4f}{g.cos_theta.median():>+9.4f}"
                  f"{g.estimated_gain.median():>10.4f}"
                  f"{g.estimated_gain.quantile(0.1):>9.4f}")
        print(f"\n  W0 per-window identity: max |pred-obs|/obs = "
              f"{(L.identity_err/L.gain_oracle_obs.abs().clip(lower=1e-30)).max():.3e}")
        if L.P_band.nunique() >= 3:
            import numpy as _np
            gr = L.groupby("P_band").P_growth.first().values
            lm = L.groupby("P_band").lam.median().values
            gn = L.groupby("P_band").estimated_gain.median().values
            def _sp(x, y):
                rx = _np.argsort(_np.argsort(x)); ry = _np.argsort(_np.argsort(y))
                return float(_np.corrcoef(rx, ry)[0, 1])
            print(f"  W1 Spearman(growth rate, lambda) = {_sp(gr,lm):+.3f}"
                  f"  -> {'PASS' if _sp(gr,lm) < -0.6 else 'FAIL'}")
            print(f"     Spearman(growth rate, gain)   = {_sp(gr,gn):+.3f}")
            best = float(_np.max(gn))
            print(f"  W2 most damped band gain = {best:.4f} vs standard 1.651"
                  f"  -> {'PASS' if best > 1.651 else 'FAIL'}")
        print("\n  W3 side by side with Kuramoto-Sivashinsky")
        print(f"     {'system / band':<28}{'lambda':>9}{'cos':>9}{'gain':>10}")
        print(f"     {'KS  unstable':<28}{0.0900:>9.4f}{-0.6157:>+9.4f}{1.0547:>10.4f}")
        print(f"     {'KS  strongly damped':<28}{1.0452:>9.4f}{-0.9993:>+9.4f}{14.3004:>10.4f}")
        for bd, g in L.groupby("P_band"):
            print(f"     {'NS  '+bd:<28}{g.lam.median():>9.4f}"
                  f"{g.cos_theta.median():>+9.4f}{g.estimated_gain.median():>10.4f}")
    ...
    if noise_sweep and all(f.exists() for f in sfiles):
        ...
        SW.to_csv(SAVE_DIR / f"m3d_PSW_{tag}_estimator_sweep.csv", index=False)
    ...
    with open(SAVE_DIR / f"m3d_PSW_{tag}_COMPLETE.txt", "w") as f:
        f.write(
    ...
    print("\nOutputs:")
    print(SAVE_DIR / f"m3d_PSW_{tag}_causal_interventions.csv")
    print(SAVE_DIR / f"m3d_PSW_{tag}_case_summary.csv")
    if noise_sweep:
        print(SAVE_DIR / f"m3d_PSW_{tag}_estimator_sweep.csv")
    print(SAVE_DIR / f"m3d_PSW_{tag}_COMPLETE.txt")

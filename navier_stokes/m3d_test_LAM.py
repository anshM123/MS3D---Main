#!/usr/bin/env python3
"""
M3D TEST LAM — measure lambda and cos(theta) on Navier-Stokes

Runs the 12 Test-X cases across two independent CUDA subprocesses.

WHY THIS EXISTS

KS reproduced the causal signature (+q_hat beats -q_hat in 12/12) but with a
gain of only ~1.035 against Navier-Stokes' 1.65. Neither the reservoir ratio R
nor the reachability fraction rho explained the gap.

The gain is EXACTLY determined by two measurable quantities. With b the
baseline future observable error and Delta = Delta(Qe) the oracle's observable
effect:

    gain = ||b|| / ||b + Delta|| = 1 / sqrt(1 + 2*lam*cos + lam^2)

    lam = ||Delta|| / ||b||                 how large the hidden contribution is
    cos = <b, Delta> / (||b|| ||Delta||)    how well it opposes the error

Algebra, not a model. Verified on KS to 2.22e-16 across 12 configurations.
Neither R nor rho appears in it.

MEASURED ON KS:   lam = 0.048 - 0.134,  cos = -0.64 to +0.12
PREDICTION FOR NS: gain 1.65 requires 1 + 2*lam*cos + lam^2 = 1/1.65^2 = 0.367
                   i.e. lam ~ 0.5-0.9 with cos ~ -0.80 to -0.88

If NS lands there, the magnitude gap is a LAMBDA gap: lambda measures how much
of the observable error is actually CAUSED by hidden-state leakage. Both
quantities are measurable a priori, before any correction is attempted, so this
is an applicability test rather than a post-hoc fit.

Gates:
  L0  identity holds: |predicted - observed| < 1e-9 on every window
  L1  NS lambda an order of magnitude above KS: median lam > 0.35
  L2  NS alignment strong: median cos < -0.6
  L3  DESCRIPTIVE: lam and cos per case, and the KS/NS contrast
"""
...
BUILD_ID = "LAM-lambda-cos-decomposition"
...
def worker_main(physical_gpu: int, precision="fp64", jvp="analytic",
    ...
    POINT_FILE = SAVE_DIR_X / f"m3d_LAM_{tag}_gpu{physical_gpu}_interventions_partial.csv"
    CASE_FILE = SAVE_DIR_X / f"m3d_LAM_{tag}_gpu{physical_gpu}_cases_partial.csv"
    SWEEP_FILE = SAVE_DIR_X / f"m3d_LAM_{tag}_gpu{physical_gpu}_sweep_partial.csv"
...
def finalize_outputs(tag="fp64_analytic", noise_sweep=True,
    ...
    pfiles = [SAVE_DIR / f"m3d_LAM_{tag}_gpu{g}_interventions_partial.csv" for g in (0, 1)]
    cfiles = [SAVE_DIR / f"m3d_LAM_{tag}_gpu{g}_cases_partial.csv" for g in (0, 1)]
    sfiles = [SAVE_DIR / f"m3d_LAM_{tag}_gpu{g}_sweep_partial.csv" for g in (0, 1)]
    ...
    RESULTS_X.to_csv(SAVE_DIR / f"m3d_LAM_{tag}_causal_interventions.csv", index=False)
    CASES_X.to_csv(SAVE_DIR / f"m3d_LAM_{tag}_case_summary.csv", index=False)
    ...
    if noise_sweep and all(f.exists() for f in sfiles):
        ...
        SW.to_csv(SAVE_DIR / f"m3d_LAM_{tag}_estimator_sweep.csv", index=False)
    ...
    with open(SAVE_DIR / f"m3d_LAM_{tag}_COMPLETE.txt", "w") as f:
        f.write(
    ...
    print("\nOutputs:")
    print(SAVE_DIR / f"m3d_LAM_{tag}_causal_interventions.csv")
    print(SAVE_DIR / f"m3d_LAM_{tag}_case_summary.csv")
    if noise_sweep:
        print(SAVE_DIR / f"m3d_LAM_{tag}_estimator_sweep.csv")
    print(SAVE_DIR / f"m3d_LAM_{tag}_COMPLETE.txt")

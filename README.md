...
| Reachability bound `gain ≤ 1/√(1−ρ²)` | 300 windows / 600 comparisons, zero violations |
...
| Scope limit, model-form error | causal signature survives 20/20; predictions become ~6x optimistic |
| Transport reachability | rho_T = 1 identically — the geometric bound is vacuous |
| Blind lambda calibration | orders unseen systems at Spearman +0.72; fails across observable class |
...
paper/          results.json (CANONICAL STATISTICS), figure generation,
                manuscript build
...
| `navier_stokes/m3d_test_F_reachability.py` | the reachability bound, 300 windows |
...
| `analysis/modelform.py` | scope limit: what happens when the equations are wrong |
| `analysis/rhoT.py` | transport-operator reachability (vacuous, and why) |
| `analysis/blind.py` | calibrate lambda on one system, predict two others blind |
...
## Running the CPU experiments

`RUN_ALL.md` lists every CPU-runnable script with its command, runtime and what
it tests.

## Reproducibility

`paper/results.json` is the **single source of truth** for every number that
appears in a figure, the README or the manuscript. `paper/figs.py` reads it and
hard-codes nothing, so a mismatch between the three is impossible by
construction. Each entry names the script and run configuration that produced
it. Regenerate all figures with:

```bash
cd paper && python figs.py
```

### Known limitations of this release

* **Raw result artifacts are not all included.** The CSV/JSON outputs of the
  GPU runs live in the run environment; the console records in `notebooks/`
  preserve the numbers for the older campaign, and `results.json` names the
  artifact for each entry. Publishing the full raw outputs is outstanding.
* **The environment is pinned but not containerised.** Exact versions are in
  `requirements.txt`; a lockfile or container image is outstanding.
* **Git history does not establish the prospective chronology.** The
  preregistered predictions in `ginzburg_landau/cgl_run.py` and `cgl2.py` were
  written before those runs, but the repository was populated in bulk, so the
  commit history alone cannot prove it. Future blind experiments should be
